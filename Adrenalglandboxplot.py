...
adrenalgland=grouped.get_group('Adrenal gland') #get gene name and abundance data just for liver

adrenalgland_abundance=adrenalgland.drop(['Tissue enriched'], axis=1) #drop enriched column as no longer needed for boxplot

adrenalgland_abundance.to_excel(r'/Users/cerysbarclay/Desktop/Liver_data5.xlsx', sheet_name="Adrenal gland") 

#STEP 4: FOR EACH TISSUE AS A GROUP, CREATE A BOXPLOT OF ABUNDANCE DATA
dataforboxplot= adrenalgland_abundance.drop(['Gene name'], axis=1) #drop gene name and index columns not needed for boxplot

ax = dataforboxplot.boxplot(figsize=(20, 8), showfliers=False, rot=90) #Get rid of outliers

ax.set_xlabel('Tissue')
ax.set_ylabel('Abundance')
ax.set_title('Tissue Specific Abundance for Genes Enriched in Adrenal Gland') 
