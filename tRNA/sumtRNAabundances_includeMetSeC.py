...
For input file "tRNAs/tRNA probes with ac and aa.xlsx", the anticodons for 
initiator Methionine and for selenocystein encoded by stop TCA have been modified 
to differentiate them from normal elongator codons. 
These can later be dropped if necessary.
Additionally, the amino acid on each tRNA was extracted and stored in the column 'amino acid'
...
df_corrected=uploadFile("tRNAs/tRNA probes with ac and aa.xlsx", index_col=0)
...
for ac in acs:
    ...
    # add amino acid encoded by anticodon to list of amino acids
    aa=df_corrected.loc[ac, 'amino acid']
    if type(aa) is str: #only one tRNA for that anticodon
        aas.append(aa)
    else:   #more than one tRNA, keep only 1 entry for list (aa is a pd.Series)
        aas.append(list(aa)[0]) 
df_summed=pd.DataFrame(s_list, columns=s_list[0].index, index=acs)
df_summed['amino acid']=aas #replace with correct amino acid string since it was modified in loop
saveToFile(df_summed, "tRNAs/SummedtRNAab - with aa, iMet SeC included2.xlsx")